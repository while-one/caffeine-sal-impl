...
static list_node_t* get_free_node(cfn_svc_collection_t *driver) {
    list_meta_t *meta = (list_meta_t *) driver->phy->user_arg;
    if (meta->count >= meta->capacity) return NULL;
    
    size_t node_size = sizeof(list_node_t) + driver->config->item_size;
    uint8_t *pool = (uint8_t *) driver->phy->buffer;
...
static cfn_hal_error_code_t port_push_back(cfn_svc_collection_t *driver, const void *item) {
    if (!driver || !item) return CFN_HAL_ERROR_BAD_PARAM;
    list_meta_t *meta = (list_meta_t *) driver->phy->user_arg;
    
    list_node_t *node = get_free_node(driver);
    if (!node) return CFN_HAL_ERROR_MEMORY_FULL;

    memcpy(node->data, item, driver->config->item_size);
    node->next = NULL;
    node->prev = meta->tail;

    if (meta->tail) meta->tail->next = node;
    meta->tail = node;
    if (!meta->head) meta->head = node;
...
static cfn_hal_error_code_t port_push_front(cfn_svc_collection_t *driver, const void *item) {
    ...
    if (meta->head) meta->head->prev = node;
    meta->head = node;
    if (!meta->tail) meta->tail = node;
...
static cfn_hal_error_code_t port_pop_front(cfn_svc_collection_t *driver, void *item_out) {
    if (!driver || !item_out) return CFN_HAL_ERROR_BAD_PARAM;
    list_meta_t *meta = (list_meta_t *) driver->phy->user_arg;
    if (!meta->head) return CFN_HAL_ERROR_NOT_FOUND;

    list_node_t *node = meta->head;
    memcpy(item_out, node->data, driver->config->item_size);

    meta->head = node->next;
    if (meta->head) meta->head->prev = NULL;
    else meta->tail = NULL;
...
static cfn_hal_error_code_t port_get_size(cfn_svc_collection_t *driver, size_t *size_out) {
    if (!driver || !size_out) return CFN_HAL_ERROR_BAD_PARAM;
    const list_meta_t *meta = (const list_meta_t *) driver->phy->user_arg;
    *size_out = meta->count;
    return CFN_HAL_ERROR_OK;
}
...
static cfn_hal_error_code_t port_insert_at(cfn_svc_collection_t *driver, size_t index, const void *item) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(index); CFN_HAL_UNUSED(item); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_pop_back(cfn_svc_collection_t *driver, void *item_out) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(item_out); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_remove_at(cfn_svc_collection_t *driver, size_t index, void *item_out) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(index); CFN_HAL_UNUSED(item_out); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_peek_back(cfn_svc_collection_t *driver, void *item_out) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(item_out); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_peek_at(cfn_svc_collection_t *driver, size_t index, void *item_out) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(index); CFN_HAL_UNUSED(item_out); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_get_capacity(cfn_svc_collection_t *driver, size_t *capacity_out) { if(capacity_out)*capacity_out=driver->phy->size; return CFN_HAL_ERROR_OK; }
static cfn_hal_error_code_t port_is_empty(cfn_svc_collection_t *driver, bool *is_empty_out) {
    if (!is_empty_out) return CFN_HAL_ERROR_BAD_PARAM;
    size_t size;
...
static cfn_hal_error_code_t port_is_full(cfn_svc_collection_t *driver, bool *is_full_out) { CFN_HAL_UNUSED(driver); if(is_full_out)*is_full_out=false; return CFN_HAL_ERROR_OK; }
