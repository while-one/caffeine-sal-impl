...
/**
 * @brief Get the abstract Temperature interface from the AHT20 composite sensor.
 *
 * @param driver Pointer to the AHT20 composite sensor structure.
 * @return Pointer to the Temperature interface, or NULL if driver is NULL.
 */
static inline cfn_sal_temp_sensor_t *cfn_sal_aht20_get_temp(cfn_sal_aht20_t *driver)
{
    return (driver == NULL) ? NULL : &driver->temp;
}

/**
 * @brief Get the abstract Humidity interface from the AHT20 composite sensor.
 *
 * @param driver Pointer to the AHT20 composite sensor structure.
 * @return Pointer to the Humidity interface, or NULL if driver is NULL.
 */
static inline cfn_sal_hum_sensor_t *cfn_sal_aht20_get_hum(cfn_sal_aht20_t *driver)
{
