...
#include "cfn_sal_temp_rmp117_port.h"
...
static cfn_hal_error_code_t port_read_celsius(cfn_sal_temp_sensor_t *driver, float *temp_out)
{
    ...
    cfn_hal_i2c_mem_transaction_t xfr = {
        .dev_addr = TMP117_I2C_ADDR, .mem_addr = TMP117_REG_TEMP_RESULT, .mem_addr_size = 1, .data = data, .size = 2
    };
...
cfn_hal_error_code_t cfn_sal_temp_rmp117_construct(cfn_sal_temp_sensor_t       *driver,
...
cfn_hal_error_code_t cfn_sal_temp_rmp117_destruct(cfn_sal_temp_sensor_t *driver)
{
    if (driver == NULL)
    {
        return CFN_HAL_ERROR_BAD_PARAM;
