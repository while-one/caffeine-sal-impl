...
{
    cfn_sal_lsm6ds3_t *lsm = get_lsm6ds3_from_base(base);
    if (!lsm) return CFN_HAL_ERROR_BAD_PARAM;
...
{
    if (!driver || !data_out) return CFN_HAL_ERROR_BAD_PARAM;
...
static cfn_hal_error_code_t lsm6ds3_accel_read_mg(cfn_sal_accel_t *driver, cfn_sal_accel_data_t *data_out)
{
    ...
    if (err == CFN_HAL_ERROR_OK)
    {
        ...
        {
            case CFN_SAL_ACCEL_RANGE_4G:  sensitivity = 0.122f; break;
            case CFN_SAL_ACCEL_RANGE_8G:  sensitivity = 0.244f; break;
            case CFN_SAL_ACCEL_RANGE_16G: sensitivity = 0.488f; break;
            default: break;
        }
...
static cfn_hal_error_code_t lsm6ds3_accel_set_range(cfn_sal_accel_t *driver, cfn_sal_accel_range_t range)
{
    ...
    switch (range)
    {
        case CFN_SAL_ACCEL_RANGE_2G:  break;
        case CFN_SAL_ACCEL_RANGE_16G: ctrl1 |= 0x04; break;
        case CFN_SAL_ACCEL_RANGE_4G:  ctrl1 |= 0x08; break;
        case CFN_SAL_ACCEL_RANGE_8G:  ctrl1 |= 0x0C; break;
    }
    err = lsm6ds3_write_reg(lsm->combined_state.phy, LSM6DS3_REG_CTRL1_XL, ctrl1);
    if (err == CFN_HAL_ERROR_OK) lsm->current_accel_range = range;
    return err;
}

static cfn_hal_error_code_t lsm6ds3_accel_read_steps(cfn_sal_accel_t *driver, uint32_t *steps)
{
    if (!driver || !steps) return CFN_HAL_ERROR_BAD_PARAM;
    cfn_sal_lsm6ds3_t *lsm = CFN_HAL_CONTAINER_OF(driver, cfn_sal_lsm6ds3_t, accel);
    uint8_t buffer[2];
...
static cfn_hal_error_code_t lsm6ds3_gyro_read_mdps(cfn_sal_gyro_sensor_t *driver, cfn_sal_gyro_data_t *data_out)
{
    ...
    if (err == CFN_HAL_ERROR_OK)
    {
        ...
        {
            case CFN_SAL_GYRO_RANGE_125DPS:  sensitivity = 4.375f; break;
            case CFN_SAL_GYRO_RANGE_500DPS:  sensitivity = 17.50f; break;
            case CFN_SAL_GYRO_RANGE_1000DPS: sensitivity = 35.0f;  break;
            case CFN_SAL_GYRO_RANGE_2000DPS: sensitivity = 70.0f;  break;
            default: break;
        }
        data_out->x = (int32_t)((float)raw.x * sensitivity);
        data_out->y = (int32_t)((float)raw.y * sensitivity);
...
{
    if (!sensor || !phy || !phy->instance) return CFN_HAL_ERROR_BAD_PARAM;
