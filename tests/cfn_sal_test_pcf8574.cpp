  ...
  protected:
    ...
    void SetUp() override
    {
        ...
        mock_i2c_api.xfr_polling =
            [](cfn_hal_i2c_t *d, const cfn_hal_i2c_transaction_t *xfr, uint32_t t) -> cfn_hal_error_code_t
        {
            (void) d;
            (void) t;
            if (xfr->tx_payload)
...
TEST_F(PCF8574Test, WriteToInputFails)
{
    ...
    EXPECT_EQ(cfn_hal_gpio_pin_write(&pcf.hal_gpio, CFN_HAL_GPIO_PIN_4, CFN_HAL_GPIO_STATE_LOW),
